...
pub fn main() !void {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (std.mem.eql(u8, arg, "--target")) {
            ...
            try out.writeAll("IntegrityGap v2.0.0\n");
            return;
    ...
    if (batch_mode) {
        if (batch_file) |bf| {
            return runBatchMode(allocator, bf, verbose, mode);
        }
    }
    ...
    return runFullAnalysis(allocator, target, max_bytes, verbose, mode, compliance_framework, json_path, dot_path, output_html, output_md, output_sarif, plain);
}
...
fn runFullAnalysis(allocator: Allocator, path: []const u8, max_bytes: usize, verbose: bool, mode: AnalysisMode, compliance_framework: ?[]const u8, json_path: ?[]const u8, dot_path: ?[]const u8, output_html: ?[]const u8, output_md: ?[]const u8, output_sarif: ?[]const u8, plain: bool) !void {
    const stderr = std.io.getStdErr().writer();
    ...
    if (compliance_framework) |fw| {
        ...
        if (json_path) |jp| try writeComplianceJson(jp, compliance_report);
        try writeComplianceStdout(compliance_report);
        return;
    }

    if (mode == .integrity_gap or mode == .all) {
        try runIntegrityGapOutput(&analysis, json_path, dot_path, plain);
    }

    if (mode == .concurrency or mode == .all) {
        var concurrency = try concurrency_analyzer.analyzeConcurrency(allocator, analysis.instructions, analysis.image, analysis.functions);
        defer concurrency.deinit(allocator);
    ...
    if (mode == .taint or mode == .all) {
        var taint = try taint_analyzer.analyzeTaint(allocator, analysis.instructions, analysis.image, analysis.functions, analysis.call_edges);
        defer taint.deinit(allocator);
    ...
    if (mode == .crypto or mode == .all) {
        var crypto = try crypto_auditor.auditCrypto(allocator, analysis.bytes, analysis.instructions, analysis.image, analysis.functions);
    ...
    if (mode == .privacy or mode == .all) {
        var privacy = try privacy_analyzer.analyzePrivacy(allocator, analysis.bytes, analysis.instructions, analysis.image, analysis.functions);
        defer privacy.deinit(allocator);
    ...
    if (mode == .compliance or mode == .all) {
        var compliance = try compliance_engine.runFullComplianceSuite(allocator, analysis.bytes, analysis.instructions, analysis.image, analysis.functions);
        defer compliance.deinit(allocator);
    ...
    if (mode == .memory or mode == .all) {
        var memory = try memory_safety.analyzeMemorySafety(allocator, analysis.bytes, analysis.instructions, analysis.image, analysis.functions);
    ...
    if (mode == .dependencies or mode == .all) {
        var deps = try dependency_checker.analyzeDependencies(allocator, analysis.bytes, analysis.image);
    ...
    if (mode == .config or mode == .all) {
        var config = try config_auditor.auditConfiguration(allocator, analysis.bytes, analysis.instructions, analysis.image, analysis.functions);
    ...
    if (mode == .all and (output_html != null or output_md != null or output_sarif != null)) {
        ...
        var report = try report_engine.generateComprehensiveReport(
            ...
            .json_verbose, &analysis, &concurrency, &taint,
            null, &crypto, &privacy, &compliance,
            &memory, &deps, &config,
        );
...
fn runIntegrityGapOutput(analysis: *const types.Analysis, json_path: ?[]const u8, dot_path: ?[]const u8, plain: bool) !void {
    if (plain) try reporter.writePlain(analysis.*);
    if (json_path) |path| try reporter.writeJson(path, analysis.*);
    if (dot_path) |path| try reporter.writeDot(path, analysis.*);
    if (!plain and json_path == null and dot_path == null) try reporter.writeJsonStdout(analysis.*);
}
...
fn runDiffMode(allocator: Allocator, target: []const u8, other: []const u8, max_bytes: usize, verbose: bool, json_path: ?[]const u8) !void {
    ...
    if (json_path) |path| try reporter.writeDiffJson(path, analysis1, analysis2, false) else try reporter.writeDiffStdout(analysis1, analysis2, false);
}
...
fn runBatchMode(allocator: Allocator, batch_file_path: []const u8, verbose: bool, mode: AnalysisMode) !void {
    ...
    while (try reader.readUntilDelimiterOrEof(&buf, '\n')) |line| {
        ...
        runFullAnalysis(allocator, trimmed, default_max_bytes, verbose, mode, null, null, null, null, null, null, true) catch |err| {
...
fn writeComplianceStdout(report: compliance_engine.ComplianceReport) !void {
    ...
    try w.print("Compliance Report: {s}\n", .{@tagName(report.framework)});
...
fn printComplianceSummary(analysis: compliance_engine.ComplianceSuite) !void {
    ...
    for (analysis.reports) |r| {
        try w.print("  [{s}] Score: {d:.1}% Passed: {} Failed: {}\n", .{
            @tagName(r.framework), r.score, r.passed, r.failed,
        });
    }
}
...
fn printUsage() void {
    const out = std.io.getStdOut().writer();
    out.writeAll(
        \\IntegrityGap - Commercial-Grade Binary Integrity Analysis Suite v2.0.0
        \\
        \\USAGE:
        \\  IntegrityGap <target> [options]
        \\
        \\ANALYSIS MODES:
        \\  --mode <mode>          Analysis mode: all, integrity-gap, concurrency, taint,
        \\                         firmware, crypto, privacy, compliance, memory,
        \\                         dependencies, config (default: all)
        \\
        \\OUTPUT OPTIONS:
        ...
        \\
        \\COMPLIANCE OPTIONS:
        \\  --compliance <fw>      Run compliance check: pci-dss, hipaa, soc2, iso-27001
        \\
        \\BATCH OPTIONS:
        \\  --batch <file>         Batch process files listed in <file>
        \\  --firmware             Firmware-specific analysis mode
